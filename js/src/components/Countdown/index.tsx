...
const CountdownDigit: FC<{ countdownProps: CountdownRenderProps }> = ({ countdownProps }) => {
  const { days, hours, minutes, seconds } = countdownProps
  const dayLength = days < 10 ? 2 : days.toString().length
  const dayArr = days.toString().padStart(2, '0').split('')
  const timeArr = `${hours.toString().padStart(2, '0')}${minutes.toString().padStart(2, '0')}${seconds.toString().padStart(2, '0')}`.split('')
...
const Countdown: FC<{ toTime: number; title: React.ReactNode }> = ({ toTime, title }) => {
  return (
    <div className="text-center my-20">
      <p className="text-2xl">{title}</p>
      <div>
        <RcCountdown date={Date.now() + toTime - Date.now()} renderer={(props) => <CountdownDigit countdownProps={props} />} />
      </div>
    </div>
  )
}
