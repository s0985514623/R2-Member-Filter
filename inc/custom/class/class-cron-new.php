...
class CronNew extends Bootstrap
{
    public function __construct()
    {
        ...
        //確保每個用戶都能有自己個Cron事件，但只有在呼叫自己的Cron事件時才會執行
				\add_action( 'plugins_loaded', [$this,'addUserCronHook'] );
        //在會員篩選頁面加入子頁面
        ...
        // //在下單後清除使用者的Cron事件
        // \add_action('woocommerce_checkout_order_processed ', array($this, 'order_completed_clear_cron'), 10, 1);
        //在訂單完成後清除使用者的Cron事件
...
<?php
     ...
     * 帶入使用者ID與新加入購物車的商品ID
     */
    public static function set_mail($userID, $product_id)
    {
        //先清除事件後再加入新事件
	...
	<div style="width:80%;text-align: left;padding:0 10px 10px;">
		<div>
			<a style="color:#4562a8;text-decoration:none;font-size:18px;font-weight: 700;"
				href="<?=$CartProduct[ 'productLink' ]?>"><?=$CartProduct[ 'productName' ]?></a>
		</div>
...
# code...
        ...
        add_action('custom_customer_details', function () use ($UserName) {
            ?>
<span
	style="color:#636363;width:100%;display:block;text-align:left;padding:15px 15px 0"><?=$UserName?>，您好</span>
<?php
}, 5);
    }
    ...
    //單除拉出清除事件方法
    public static function clear_cron($userID)
    {
