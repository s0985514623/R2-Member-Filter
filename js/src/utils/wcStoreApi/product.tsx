...
export const getVariationIdByAttributes = (product: TProduct, attributes: TProductVariationAttribute[]) => {
    const variations = product?.variations ?? []
    const theVariation = variations.find((v) => {
        const theAttributes = v?.attributes ?? []
        const theAttributesWithoutAny = theAttributes.filter((a) => a.value !== null)
